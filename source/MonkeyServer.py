...
class ScreenShot(threading.Thread):
    ...
    def addpath(self, path):
        self.__path = path + '/screenshot/shot'
        self.__logpath = path + 'log_new.txt'
            ...
            for op in self.__oplist:
                ...
                if op.optype == 'touch':
                    ...
                    while (num <= touch_number):
                        if self.__running.isSet():
                            self.__flag.wait()
                            self.writelog(
                                "%stouch %d (%d,%d)." % (time.strftime("%Y-%m-%d %H:%M:%S "), num, pos_x, pos_y))
                ...
                # random_touch
                    ...
                    while (num <= touch_number):
                        if self.__running.isSet():
                            ...
                            self.writelog(
                                "%srandom_touch %d (%d,%d)." % (time.strftime("%Y-%m-%d %H:%M:%S "), num, x, y))
                ...
                # drag
                    ...
                    while (num <= drag_number):
                        if self.__running.isSet():
                            self.__flag.wait()
                            self.writelog("%sdrag %d (%d,%d) to (%d,%d)." % (
                            time.strftime("%Y-%m-%d %H:%M:%S "), num, start_x, start_y, end_x, end_y))
                ...
                # random_drag
                    ...
                    while (num <= drag_number):
                        if self.__running.isSet():
                            ...
                            self.writelog("%srandom_drag %d (%d,%d) to (%d,%d)." % (
                            time.strftime("%Y-%m-%d %H:%M:%S "), num, x_start, y_start, x_end, y_end))
